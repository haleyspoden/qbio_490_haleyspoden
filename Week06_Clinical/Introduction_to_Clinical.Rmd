...
```{r}
write.csv(clinic, "/home1/spoden/490_cluster/analysis_data/brca_clinical_data.csv", row.names=FALSE)
```

To read the csv back into your environment as a dataframe:

```{r}
clinic <- read.csv("/home1/spoden/490_cluster/analysis_data/brca_clinical_data.csv")
```
...
plot(
  ...
  main = "Last Contact Days To vs. Lymph Nodes Examined Count",
...
# create plot here
boxplot(
  clinic$age_at_diagnosis ~ clinic$tumor_status,
  main = "Age at Diagnosis by Tumor Status",
  xlab = "Tumor Status",
...
# 5. What is the approx. 15-year survival rate for "Old" breast cancer patients? 0.30

# 6. What is the p-value? Is it statistically significant and what does that tell us about the relationship between age and breast cancer survival? The p-value is 0.2 which is not statistically significant and therefore tells us that there is not a strong relationship between age and breast cancer survival. 
